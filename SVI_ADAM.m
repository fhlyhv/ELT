function [XO,lv] = SVI_ADAM(XMid,XO,Bopt,rhov,nuv,typv,Gh,Sh,Lh,pc)

% pc -- number of the variables in the coarser scale
...
UO = [normcdf(XO(1:pc)),gevcdf(XO(pc+1:end),Gh,Sh,Lh)]';
...
GM = Gh(XMid(pc+1:end)-pc).';
SM = Sh(XMid(pc+1:end)-pc).';
LM = Lh(XMid(pc+1:end)-pc).';
...
for nt = 1:1e7
    ...
    utmp = [normcdf(Xtmp(1:pc));gevcdf(Xtmp(pc+1:end),GM,SM,LM)];
    utmp(utmp==0) = 1e-6;
    utmp(utmp==1) = 1-1e-6;
    UO(XMid) = utmp;
    for i = typset
    ...
    pfpx = [-Xtmp(1:pc);-(1+GM)./SM./a0 + a0.^(-1./GM-1)./SM];
    pupx = [normpdf(Xtmp(1:pc));gevpdf(Xtmp(pc+1:end),GM,SM,LM)];
    ...
    if rem(nt,1e3) == 0
        mh = m;
        RMAE_m = mean(abs(mh-m0))/mean(abs(m0));
        fprintf('# iterations = %d, RMAE_m = %d\n',nt,RMAE_m);
        if RMAE_m < tol
            t = toc;
            break;
        else
